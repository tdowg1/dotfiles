 ...
 * new cfdump ( $strXml, "xml" );
 * 
\*********************************************************************************************************************/

function cfdump($var2cfdump){
	echo "<h5>say 'new cfdump(var)' to see variable names</h5>";
	new cfdump($var2cfdump);
}
...
class cfdump {
	...
	//constructor
	function cfdump($var,$forceType="",$bCollapsed=false) {
		//include js and css scripts
		if(!defined('BCFDUMPINIT')) {
			define("BCFDUMPINIT", TRUE);
			$this->initJSandCSS();
		}
	...
	function getVariableName() {
		...
		for($i=count($arrBacktrace)-1; $i>=0; $i--) {
			...
			if(array_key_exists("function", $arrCurrent) && 
				(in_array($arrCurrent["function"], $arrInclude) || (0 != strcasecmp($arrCurrent["function"], "cfdump"))))
				continue;
		...
		if(isset($arrFile)) {
			...
			//find call to cfdump class
			preg_match('/\bnew cfdump\s*\(\s*(.+)\s*\);/i', $code, $arrMatches);
	...
	function makeTableHeader($type,$header,$colspan=2) {
		...
		echo "<table cellspacing=2 cellpadding=3 class=\"cfdump_".$type."\">
				<tr>
					<td ".$str_i."class=\"cfdump_".$type."Header\" colspan=".$colspan." onClick='cfdump_toggleTable(this)'>".$header."</td>
				</tr>";
	}
	...
	function makeTDHeader($type,$header) {
		...
		echo "<tr".$str_d.">
				<td valign=\"top\" onClick='cfdump_toggleRow(this)' class=\"cfdump_".$type."Key\">".$header."</td>
				<td>";
	}
	...
	function varIsDBResource($var,$db="mysql") {
		...
		echo "<tr><td class=\"cfdump_resourceKey\">&nbsp;</td>";
		for($i=0;$i<$numfields;$i++) {
			...
			echo "<td class=\"cfdump_resourceKey\" title=\"".$field_header."\">".$field_name."</td>";
		}
		echo "</tr>";
		for($i=0;$i<$numrows;$i++) {
			$row=call_user_func($db."_fetch_array",$var,constant(strtoupper($db)."_ASSOC"));
			echo "<tr>\n";
			echo "<td class=\"cfdump_resourceKey\">".($i+1)."</td>"; 
	...
	function initJSandCSS() {
		echo <<<SCRIPTS
			...
			<style type="text/css">
				table.cfdump_array,table.cfdump_object,table.cfdump_resource,table.cfdump_resourceC,table.cfdump_xml {
					font-family:Verdana, Arial, Helvetica, sans-serif; color:#000000; font-size:12px;
				}
				
				.cfdump_arrayHeader,
				.cfdump_objectHeader,
				.cfdump_resourceHeader,
				.cfdump_resourceCHeader,
				.cfdump_xmlHeader 
				...
				/* array */
				table.cfdump_array { background-color:#006600; }
				table.cfdump_array td { background-color:#FFFFFF; }
				table.cfdump_array td.cfdump_arrayHeader { background-color:#009900; }
				table.cfdump_array td.cfdump_arrayKey { background-color:#CCFFCC; }
				
				/* object */
				table.cfdump_object { background-color:#0000CC; }
				table.cfdump_object td { background-color:#FFFFFF; }
				table.cfdump_object td.cfdump_objectHeader { background-color:#4444CC; }
				table.cfdump_object td.cfdump_objectKey { background-color:#CCDDFF; }
				
				/* resource */
				table.cfdump_resourceC { background-color:#884488; }
				table.cfdump_resourceC td { background-color:#FFFFFF; }
				table.cfdump_resourceC td.cfdump_resourceCHeader { background-color:#AA66AA; }
				table.cfdump_resourceC td.cfdump_resourceCKey { background-color:#FFDDFF; }
				
				/* resource */
				table.cfdump_resource { background-color:#884488; }
				table.cfdump_resource td { background-color:#FFFFFF; }
				table.cfdump_resource td.cfdump_resourceHeader { background-color:#AA66AA; }
				table.cfdump_resource td.cfdump_resourceKey { background-color:#FFDDFF; }
				
				/* xml */
				table.cfdump_xml { background-color:#888888; }
				table.cfdump_xml td { background-color:#FFFFFF; }
				table.cfdump_xml td.cfdump_xmlHeader { background-color:#AAAAAA; }
				table.cfdump_xml td.cfdump_xmlKey { background-color:#DDDDDD; }
			</style>
SCRIPTS;
	}
